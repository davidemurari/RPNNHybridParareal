...
if __name__ == "__main__":
    ...
    parser.add_argument("--coarse_factors", type=str, default="5,10,20,40", help="Comma-separated factors for classical coarse dt: dt_coarse = factor * dt_fine.")
    ...
    rpnn_time = {f: [] for f in factors}
    rpnn_speedup = {f: [] for f in factors}
    rpnn_iters = {f: [] for f in factors}
    rpnn_e0 = {f: [] for f in factors}
    rpnn_efinal = {f: [] for f in factors}
    ...
    total_solves = args.num_trials * (2 * len(factors))
    with tqdm(total=total_solves, desc="Comparison progress", unit="solve", disable=args.no_progress) as pbar:
        for trial in range(args.num_trials):
            ...
            # True fine reference on a dense/fine grid, reused for every coarse factor in this trial.
            time_fine, sol_fine = compute_solution_on_uniform_grid(y0, t_final, vec_ref.dt_fine, vec_ref)

            for factor in factors:
                time_factor, dts_factor = build_coarse_grid(t_final, vec_ref.dt_fine, factor)
                ...
                rpnn_time[factor].append(total_rpnn)
                rpnn_speedup[factor].append(serial_time / total_rpnn)
                rpnn_iters[factor].append(len(diag_rpnn["error_history"]))
                rpnn_e0[factor].append(diag_rpnn["error_history"][0])
                rpnn_efinal[factor].append(diag_rpnn["error_history"][-1])
                pbar.set_postfix_str(f"trial {trial + 1}/{args.num_trials} RPNN x{factor:g}")
                pbar.update(1)
    ...
    report_path = os.path.join(reports_dir, f"compare_parareal_{args.system}_{args.nodes}.txt")
    csv_path = os.path.join(reports_dir, f"compare_parareal_{args.system}_{args.nodes}.csv")
    speedup_plot_path = os.path.join(plots_dir, f"compare_speedup_{args.system}_{args.nodes}.pdf")
    iter_plot_path = os.path.join(plots_dir, f"compare_iterations_{args.system}_{args.nodes}.pdf")

    with open(report_path, "w") as f:
        f.write("============================================================\n")
        f.write("Classical vs RPNN Parareal comparison\n")
        f.write("============================================================\n")
        ...
        f.write(f"coarse_factors: {factors}\n\n")

        f.write("RPNN Parareal by coarse factor\n")
        f.write("factor, dt_target, total_time_mean, total_time_std, speedup_mean, speedup_std, ")
        f.write("iters_mean, iters_std, E0_mean, E0_std, Efinal_mean, Efinal_std\n")
        for factor in factors:
            m_t, s_t = summarize(rpnn_time[factor])
            m_su, s_su = summarize(rpnn_speedup[factor])
            m_it, s_it = summarize(rpnn_iters[factor])
            m_e0, s_e0 = summarize(rpnn_e0[factor])
            m_ef, s_ef = summarize(rpnn_efinal[factor])
            f.write(
        ...
        f.write("\n")
        f.write("Classical Parareal by coarse factor\n")
        f.write("factor, dt_coarse, total_time_mean, total_time_std, speedup_mean, speedup_std, ")
        f.write("iters_mean, iters_std, E0_mean, E0_std, Efinal_mean, Efinal_std\n")
        for factor in factors:
    ...
    rpnn_speedup_mean = np.asarray([summarize(rpnn_speedup[f])[0] for f in factors], dtype=float)
    rpnn_speedup_std = np.asarray([summarize(rpnn_speedup[f])[1] for f in factors], dtype=float)
    ...
    plt.errorbar(factor_arr, rpnn_speedup_mean, yerr=rpnn_speedup_std, marker="s", capsize=3, label="RPNN Parareal")
    plt.xlabel("Coarse factor (dt_coarse / dt_fine)")
    plt.ylabel("Speedup")
    plt.title(f"Parareal Speedup Comparison ({args.system}, {args.nodes})")
    ...
    rpnn_iters_mean = np.asarray([summarize(rpnn_iters[f])[0] for f in factors], dtype=float)
    rpnn_iters_std = np.asarray([summarize(rpnn_iters[f])[1] for f in factors], dtype=float)

    plt.figure(figsize=(9, 5))
    plt.errorbar(factor_arr, classical_iters_mean, yerr=classical_iters_std, marker="o", capsize=3, label="Classical Parareal")
    plt.errorbar(factor_arr, rpnn_iters_mean, yerr=rpnn_iters_std, marker="s", capsize=3, label="RPNN Parareal")
    plt.gca().xaxis.set_major_locator(MaxNLocator(integer=True))
    plt.xlabel("Coarse factor (dt_coarse / dt_fine)")
    plt.ylabel("Parareal iterations")
    plt.title(f"Parareal Iteration Count Comparison ({args.system}, {args.nodes})")
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
